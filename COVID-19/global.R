...
url_path = "mongodb+srv://coviduser:test123@cluster0-cosey.gcp.mongodb.net/test"


# Connect to the database and the desired collection as coviduser:
confirmed <- mongo(collection = "confirmed", db = "covid", url = url_path)
deaths <- mongo(collection = "deaths", db = "covid", url = url_path)
recovered <- mongo(collection = "recovered", db = "covid", url = url_path)
testing <- mongo(collection = "testing", db = "covid", url = url_path)
# reading from Mongodb
data_confirmed <- confirmed$find('{}')
data_deceased <- deaths$find('{}')
data_recovered <- recovered$find('{}')
data_testing <- testing$find('{}')
...
#data_testing <- read_csv("data/owid-covid-data.csv")
# Get latest data
...
top5_countries_testing <- data_testing %>%
  filter(date == current_date) %>%
  group_by(location) %>%
  summarise(value = mean(total_tests_per_thousand, na.rm = T)) %>%
  arrange(desc(value)) %>%
  top_n(5) %>%
  select(location) %>%
  pull()
