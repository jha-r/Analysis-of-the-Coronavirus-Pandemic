...
output$case_testing <- renderPlotly({
  req(input$Testing_country)
  data <- data_testing %>%
    mutate(location = sapply(location, capFirst)) %>%
    filter(if (input$Testing_country == "United States") TRUE else location %in% input$Testing_country) %>%
 # data <- data_testing %>%
  ...
  p <- data %>%plot_ly(
    ...
    name  = sapply(data$location, capFirst),
    color = ~location,
    colors = "Dark2",
    type  = 'scatter',
    mode  = 'lines') %>%
    layout(
      yaxis = list(title = "Test/1000"),
      xaxis = list(title = "Date")
    )
  
})

output$selectize_testingByCountries <- renderUI({
  selectizeInput(
    "Testing_country",
    label    = "Select Countries",
    choices  = unique(data_testing$location),
    selected = top5_countries_testing,
    multiple = TRUE
  )
})

output$case_confirmvstesting <- renderPlotly({
  data <- data_testing %>%
   mutate(location = sapply(location, capFirst)) %>%
    group_by(location) %>% 
    select (location,date,total_cases,total_tests_per_thousand) %>%
    na.omit() %>%
    as.data.frame()
  ...
  p <- plot_ly(data,
    x     = ~total_cases,
    y     = ~1000*total_tests_per_thousand,
   # name  = sapply(data$location, capFirst),
    color = ~location,
    colors = "Dark2",
    type  = 'scatter',
    mode = 'marker'  ) %>%
   add_text(text = ~sapply(data$location, capFirst),
                 textposition = 'middle right') %>%
    layout(showlegend=FALSE,
      yaxis = list(title = "Testing",type = "log"),
...
output$case_evolution_byCountry <- renderPlotly({
  ...
  p <- plot_ly(data = data$confirmed, x = ~date, y = ~Confirmed, color = ~`Country/Region`, colors = "Dark2",type = 'scatter', mode = 'lines',
    legendgroup     = ~`Country/Region`) %>%
    add_trace(data = data$recovered, x = ~date, y = ~`Estimated Recoveries`, color = ~`Country/Region`,colors = "Dark2", line = list(dash = 'dash'),
    ...
    add_trace(data = data$confirmed[which(data$confirmed$`Country/Region` == input$caseEvolution_country[1]),],
      x            = ~date, y = -100, line = list(color = 'rgb(1, 1, 1)'), legendgroup = 'helper', name = "Confirmed") %>%
    add_trace(data = data$confirmed[which(data$confirmed$`Country/Region` == input$caseEvolution_country[1]),],
      x            = ~date, y = -100, line = list(color = 'rgb(1, 1, 1)', dash = 'dash'), legendgroup = 'helper', name = "Estimated Recoveries") %>%
    add_trace(data = data$confirmed[which(data$confirmed$`Country/Region` == input$caseEvolution_country[1]),],
      x            = ~date, y = -100, line = list(color = 'rgb(1, 1, 1)', dash = 'dot'), legendgroup = 'helper', name = "Deceased") %>%
    layout(
...
output$case_evolution_new <- renderPlotly({
  ...
  p <- plot_ly(data = data, x = ~date, y = ~new_cases, color = ~var,colors = "Dark2", type = 'bar') %>%
    layout(
...
output$case_evolution_after100 <- renderPlotly({
  ...
  p <- plot_ly(data = data, x = ~daysSince, y = ~value, color = ~`Country/Region`,colors = "Dark2", type = 'scatter', mode = 'lines')
...
output$box_caseEvolution <- renderUI({
  tagList(
    fluidRow(
      ...
      box(
        title = "Testing Per Thousand",
        plotlyOutput("case_testing"),
        column(
          uiOutput("selectize_testingByCountries"),
          width = 3
        ),column(
          HTML("Note: Testing data source: https://github.com/owid/covid-19-data/tree/master/public/data"),
          width = 9
        ),
        width = 6
      ),
      box(
        title = "Testing v/s Confirmed Case",
        plotlyOutput("case_confirmvstesting",height = 500)
      ),
        width = 6
      ),
     ...
     fluidRow( 
      ...
      box(
        ...
        column(
          HTML("Note: Active cases are calculated as <i>Confirmed - (Estimated Recoveries + Deceased)</i>"),
          width = 7
        ),
        width = 6
