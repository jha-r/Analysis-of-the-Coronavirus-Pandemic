...
series_data_dir = './data'

######## Point to your data directory 
series_all_files = list.files(series_data_dir)
series_data_files = series_all_files[grepl('global.csv', series_all_files)]
...
format_date = function(dat, date_var){
  ...
  mon_c = ifelse(mon_n == '1', 'Jan', ifelse(mon_n == '2', 'Feb',ifelse(mon_n == '3',  'Mar',ifelse(mon_n == '4','Apr','May'))))
...
anim = staticplot + 
  ...
  labs(title = 'Spead of Confirmed Cases per day: {closest_state}',
       ...
       caption = sprintf("Data Source:John Hopkins Data Repo\n Data Extracted at %s ", 
...
if(output_type == 'GIF'){  ### Save as GIF
  ...
  animate(anim, 500, fps = animate_speed, 
          width = 1500, height = 1000, end_pause = 1, start_pause = 1, rewind=FALSE,
